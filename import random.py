...
ANNUAL_VOLAT = np.std(data['Close']) * np.sqrt(4) / 100                          # Annual volatility in stock price
BID_ASK_SP = np.mean(data['High'] - data['Low'])                                 # Bid-ask spread
DAILY_TRADE_VOL = average_daily_volume                                           # Average Daily trading volume  
TRAD_DAYS = len(data)                                                            # Number of trading days in a year
...
TOTAL_SHARES = 250000                                                # Total number of shares to sell
STARTING_PRICE =  data["Close"].iloc[0]                              # Starting price per share
LLAMBDA = 1e-8                                                       # Trader's risk aversion
