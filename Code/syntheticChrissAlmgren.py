...
ANNUAL_VOLAT = 1                                # Annual volatility in stock price
BID_ASK_SP = 0.0625                                 # Bid-ask spread
DAILY_TRADE_VOL = 5e6                              # Daily trading volume  
TRAD_DAYS = 250                                    # Number of trading days in a year
DAILY_VOLAT = 0.95   # Daily volatility in stock price
...
LLAMBDA = 2*10**(-15)                                                       # Trader's risk aversion
LIQUIDATION_TIME = 5                                                # How many days to sell all the shares. 
NUM_N = 5                                                           # Number of trades
EPSILON = 0.00625                                        # Fixed Cost of Selling.
SINGLE_STEP_VARIANCE = 0.95          # Calculate single step variance
ETA = 2.5 * 10**(-6)                         # Price Impact for Each 1% of Daily Volume Traded
GAMMA = 2.10 * 10**(-7)                       # Permanent Impact Constant
...
#             print('in here')
            ...
            info.expected_variance = self.singleStepVariance * self.tau * self.totalSRSQ * (1+np.random.standard_t(1)/100)
...
#             self.k += 1
            ...
            #info.price = self.prevImpactedPrice + self.singleStepVariance * self.tau * random.normalvariate(0,1)
            
            info.price = self.prevImpactedPrice + np.sqrt(self.singleStepVariance * self.tau) * (1+np.random.standard_t(1)/100)
