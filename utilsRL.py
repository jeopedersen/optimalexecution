...
def generate_table(left_col, right_col, table_title):
    ...
    if right_col:
        ...
        # Generate the right table
        gen_stubs_right, gen_data_right = it.zip_longest(*right_col)
    ...
    gen_stubs_left, gen_data_left = it.zip_longest(*left_col) 
...
def implement_trade_list(seed = 42, lq_time = 60, nm_trades = 60, tr_risk = 1e-6):
...
def get_av_std(lq_time = 60, nm_trades = 60, tr_risk = 1e-6, trs = 100, seed = 42):
    random.seed(seed)
    # Create simulation environment
    env = sca.MarketEnvironment()
    ...
    plt.plot(shortfall_hist, label='')
    plt.xlim(0, trs)
    ax = plt.gca()
    #ax.set_facecolor('k')
    ax.set_xlabel('Episode', fontsize = 15)
    ax.set_ylabel('Implementation Shortfall ($)', fontsize = 15)
    ax.axhline(shortfall_hist.mean(),0, 1, color = 'm', label='Average')
    yNumFmt = mticker.StrMethodFormatter('${x:,.0f}')
    ax.yaxis.set_major_formatter(yNumFmt)
    plt.legend()
    plt.show
    return shortfall_hist

def get_av_std1(lq_time=60, nm_trades=60, tr_risk=1e-6, trs=100, seed=42):
    random.seed(seed)
    # Create simulation environment
    env = sca.MarketEnvironment()
    ...
    trimmed_shortfall_hist = stats.trim_mean(shortfall_hist, 0.)  # Trim 5% from both tails
    std = np.std(trimmed_shortfall_hist)

    print('Average Implementation Shortfall: ${:,.2f}'.format(shortfall_hist.mean()))
    print('Standard Deviation of the Implementation Shortfall: ${:,.2f}'.format(std))
    plt.plot(shortfall_hist, label='')
    plt.xlim(0, trs)
    ax = plt.gca()
    # ax.set_facecolor('k')
